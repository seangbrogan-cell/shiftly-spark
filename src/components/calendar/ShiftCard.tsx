...
export function ShiftCard({ assignment, onDelete }: ShiftCardProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`group relative rounded border ${color.bg} ${color.border} px-0.5 sm:px-1.5 py-1 @container cursor-grab active:cursor-grabbing transition-shadow hover:shadow-md flex-1 flex items-center ${isDragging ? 'shadow-lg ring-2 ring-primary/30' : ''}`}
    >
